...
void computeMatricesFromInputs(int i){
...
//    ypos = yheight/2;
	...
	horizontalAngle += mouseSpeed * float(xwidth/2 - xpos );
	verticalAngle   += mouseSpeed * float( yheight/2 - ypos );
    ...
    //position = glm::vec3(0,0,4) ;//+ glm::vec3( 1 * cos(deltaTime), 1, 1 * sin(deltaTime) ); //position,           // Camera is here
